...
export class MailService {
  ...
  async sendOrderStatusUpdate(to: string, name: string, orderNumber: string, status: string, language: string = 'en'): Promise<void> {
    // Always send bilingual email: Arabic first, then English
    ...
    const html = `
      <div style="max-width:600px;margin:0 auto;font-family:Arial,sans-serif;background:#fff;border:1px solid #e5e7eb;border-radius:16px;overflow:hidden;">
        <div style="background:linear-gradient(135deg,#1a1a2e,#16213e);padding:32px;text-align:center;">
          <h1 style="color:#BA974F;margin:0;font-size:24px;">${this.brandName}</h1>
          <p style="color:#94a3b8;margin:8px 0 0;">تحديث الطلب / Order Update</p>
        </div>

        <!-- Arabic section -->
        <div style="padding:24px;direction:rtl;text-align:right;border-top:1px solid #eef2f7;">
          <h2 style="color:#1a1a2e;margin:0 0 12px;">مرحبا ${name},</h2>
          <p style="color:#64748b;line-height:1.6;">تحديث حالة الطلب <strong>#${orderNumber}</strong>:</p>
          <div style="background:#f8fafc;border-right:4px solid #BA974F;padding:12px;margin:16px 0;border-radius:8px 0 0 8px;">
          ...
          <p style="color:#94a3b8;font-size:12px;margin-top:8px;">شكراً لاختيارك عود الزباره 🌿</p>
        </div>

        <!-- English section -->
        <div style="padding:24px;border-top:1px solid #eef2f7;">
          <h2 style="color:#1a1a2e;margin:0 0 12px;">Hello ${name},</h2>
          <p style="color:#64748b;line-height:1.6;">Order <strong>#${orderNumber}</strong> status update:</p>
          <div style="background:#f8fafc;border-left:4px solid #BA974F;padding:12px;margin:16px 0;border-radius:0 8px 8px 0;">
            <strong style="color:#1a1a2e;text-transform:capitalize;">${status}</strong>
            <p style="color:#64748b;margin:8px 0 0;">${statusMessagesEn[status] || 'Your order status has been updated.'}</p>
          </div>
          <p style="color:#94a3b8;font-size:12px;margin-top:8px;">Thank you for choosing Oud Al Zubarah 🌿</p>
        </div>
      </div>
    `;
